...
(defun compare-users (uid1 uid2 &optional (ratinglist2 (find-ratings-by-uid uid2)))
  (declare (optimize (speed 3) (safety 0))
	   (fixnum uid1 uid2))
  ...
  (let ((ratinglist1 (find-ratings-by-uid uid1)))
...
(defun find-best-correlations (uid other-uids cur-mid)
  (declare (optimize (speed 3) (safety 0))
	   (fixnum uid))
  "Returns a vector of #(rms-error uid), sorted by rmse, decreasing"
  (let* ((rating-list (delete cur-mid (find-ratings-by-uid uid) :key 'extract-mid :count 1))
...
(defun predict-user-rating (uid mid &optional (users-who-rated (find-users-who-rated mid)))
  (let* ((others-who-rated (remove uid users-who-rated :count 1))
	 (full-correlations (find-best-correlations uid others-who-rated mid))
