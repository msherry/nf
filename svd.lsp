;; TODO - find ways to balance movies with few ratings that are still
;; rated highly (e.g. Mobsters and Mormons). Perhaps change the
;; learning rate.
...
(defun fudge-first-feature ()
  (let ((averages (loop for movie from 1 to +num-movies+ collect
			(average-movie-stars movie))))
    (vector-push-extend (make-array (1+ +num-movies+) 
				    :initial-contents (cons 1.0 averages)) *movie-features*)
    (vector-push-extend (make-array +num-users+ :initial-element 1.0) *user-features*)))
...
(defun train-all-features (&key (max-feature 9) (epochs 25) (step 10) (rate .001) (show-rmse T))
  (gc :full t)
  (let ((rating-cache (make-array (length *uid-sorted*) :initial-element 0.0 :adjustable nil :fill-pointer nil)))
    ...
    (loop for feature from (or (length *user-features*) 0) to max-feature do
	  ...
	  (loop for epoch from 1 to (+ epochs (* feature step)) do
		(train-feature feature rating-cache rate)
		(when (zerop (mod epoch 10))
		  (format t "Epoch: ~a/~a~%" epoch (+ epochs (* feature step)))))
...
(defun train-feature (feature rating-cache &optional (rate .001))
  (declare (optimize (speed 3) (safety 0))
	   (fixnum feature)
	   (single-float rate)
	   (type (simple-array fixnum) *uid-sorted*)
  ...
  (let ((u-f (aref *user-features* feature))
    ...
    (loop for r fixnum across *uid-sorted*
	  ...
	  (let* ((mid (extract-mid r))
		 ...
		 (mv (svref m-f mid))
		 (err (* rate (- (the fixnum (extract-stars r))
...
(declaim (inline caching-predict-user-rating))
(defun caching-predict-user-rating (uv*mv rating-cache rating-num)
  ...
  (let ((predicted (+ uv*mv
    ...
    (clip-result predicted)))
    ;; Try it without clipping intermediate results
    ;predicted))
...
(defun predict-user-rating (uid-index mid)
  ...
  (loop for u-f simple-vector across *user-features*
	...
	(return (clip-result result))))

(declaim (inline clip-result))
(defun clip-result (result)
  (declare (optimize (speed 3) (safety 0))
	   (single-float result))
  (cond ((> result 5.0) 5.0)
	((< result 1.0) 1.0)
...
(defun fast-find (item sequence &optional (start 0) (end (1- (length sequence))))
  "Let's not count on being passed a simple array, in case we can ever
  make displaced arrays efficient."
  (declare (optimize (speed 3) (safety 0))
 	   (fixnum start end item)
 	   (type (array fixnum) sequence))
  (unless (< end start)
    (let* ((mid (truncate (/ (the fixnum (+ end start)) 2)))
	   (rating (aref sequence mid))
	   (uid (extract-uid rating)))
      (declare (fixnum uid))
      (cond ((> uid item) (fast-find item sequence start (1- mid)))
	...
	(loop for line = (read-line s nil nil)
	      while (and line
			 (or nolimit
			     (< num-processed count)))
	      do
	      (cond ((char= #\: (schar line (1- (length line)))) ;test last char for ":"
...
(defun find-movies-by-feature (feature &key (num 40) (fn #'>))
  (map 'vector #'(lambda(x) (vector (find-movie-title (svref x 0)) (svref x 1)))
...
(defun find-users-by-feature (feature &key (num 40) (fn #'>))
  "Gives the user index, not the actual UID."
  (subseq (sort (loop for i from 0 below +num-users+ collect
		      (vector i (svref (aref *user-features* feature) i)))
		fn
		:key #'(lambda(x) (svref x 1)))
	  0 num))
...
(defun strength-of-movie-feature (num)
  ;; Have to remove the dummy "1.0" at the beginning of *movie-features*
  (sqrt (/ (1- (loop for i across (aref *movie-features* num) sum (* i i)))
	   +num-movies+)))

(defun strength-of-user-feature (num)
  (sqrt (/ (loop for i across (aref *user-features* num) sum (* i i))
	   +num-users+)))

(defun strength-table ()
  (format t "Feature:   Movie feature   User feature    Sum~%")
  (format t "~{~{    ~,8A      ~,8F      ~,8F      ~,8F ~}~%~}"
	   (loop for m-f across *movie-features*
		 for u-f across *user-features*
		 for i from 0
		 collect
		 (let ((mf-s (strength-of-movie-feature i))
		       (uf-s (strength-of-user-feature i)))
		   (list i mf-s uf-s (+ mf-s uf-s))))))
